package com.caoweiwei.cms.utils;
...
//	  添加了文章状态的分页
	  public static String page2(int page, int pages, String url, int pageSize,int status)
      {
          if (page< 1) page= 1;
          if (page> pages) page= pages;
          // 根据url是否参数,改变拼接的符合
          ...
          sb.append("<nav aria-label='Page navigation example'>");
          //'url?page=1'   ,url, flag,"page" ,page- 1 < 1 ? 1 : page- 1)
          sb.append(String.format("<ul class='pagination'><li class='page-item'><a class='page-link'  href='javascript:void(0)' data='%1$s%2$s%3$s=%4$s%5$s%6$s=%7$s' aria-label='Previous'>  <span aria-hidden='true'>&laquo;</span></a></li>" ,url, flag,"page" ,page- 1 < 1 ? 1 : page- 1,"&","status",status));
          //设开始页为1
          int beginPage = 1;
          //中间页为 pageSize/2
          int midPage = pageSize / 2;
          ...
          {                                                                                                              
        	  //当前页不加链接
              sb.append(String.format("<li class='page-item'><a  class='page-link'  href='javascript:void(0)' data='%1$s%2$s%3$s=%4$s%5$s%6$s=%7$s' >%4$s</a></li>" ,url, flag,"page" ,beginPage + i,"&","status",status));
              //如果后面的页数大于总页数，退出循环
              if (beginPage + i >= pages)
              {
                  break;
          ...
          sb.append(String.format("<li class='page-item'><a  class='page-link' href='javascript:void(0)' data='%1$s%2$s%3$s=%4$s%5$s%6$s=%7$s' aria-label='Next' ><span aria-hidden='true'>&raquo;</span></a></li>" ,url, flag,"page" ,page+ 1 >= pages ? pages : page+ 1,"&","status",status));
          sb.append("</ul></nav>");
          return sb.toString();
      }
}
